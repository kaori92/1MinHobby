...
class ModelPreferences(c: Context) {
    ...
    fun <T> getObject(key: String, c: Class<T>): T? {        //We read JSON String which was saved.
        ...
        if (value != null) {           //JSON String was found which means object can be read.           //We convert this JSON String to model object. Parameter "c" (of
         //   type Class<T>" is used to cast.
            return gson.fromJson(value, c)
        } else {            //No JSON String with this key was found which means key is invalid or object was not saved.
