...
class HobbyMatcher {
    ...
    fun match(allHobbies: MutableList<Hobby>): List<Hobby>{
        return allHobbies.filter {
            it.category == category || it.time == time
                    || it.price == price
        }
    }
     ...
     fun loadHobbies(context: Activity): MutableList<Hobby> {
        ...
        val hobbies = context.application.assets.open(fileName).bufferedReader().useLines {
                lines -> lines.forEach {

            val values: Array<String>? = it.split(",").toTypedArray()
            values?.let {

                it.forEach { Log.d("TAG123", it) }
                if(it.size == 4){
    ...
    private fun matchHobbies(type: String, time: String, price: String){
